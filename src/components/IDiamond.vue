  ...
  methods: {
    ...
    convertAttrToStyleObject() {
      ...
      this.$nextTick(() => {
        ...
        let vv = 1
        let xValue = this.propData.borderWidth, yValue = this.propData.borderWidth
        //  横向
        if (width > height) {
          vv = height / width
          xValue *= vv
        ...
        window.IDM.setStyleToPageHead(this.moduleObject.id + '::before', {
          ...
          'clip-path': `polygon(50% ${xValue}px, calc(100% - ${yValue}px) 50%, 50% calc(100% - ${xValue}px), ${yValue}px 50%)`,
        });
        this.initData();
